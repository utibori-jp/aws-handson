"""
isolate_ec2.py — guardduty-and-remediation
...
5. SNS で修復完了通知を送信する
6. 修復結果を CloudWatch Logs に記録する（監査証跡）
...
def lambda_handler(event, context):
    ...
    message = (
        f"[Auto-Remediation] EC2 instance isolated.\n"
        f"InstanceId: {instance_id}\n"
        f"QuarantineSG: {quarantine_sg_id}\n"
        f"FindingType: {finding_type}\n"
        f"FindingId: {finding_id}\n"
        f"Note: All network traffic blocked. Review instance before terminating."
    )
    _publish_sns(subject="[GuardDuty Remediation] EC2 Instance Isolated", message=message)
