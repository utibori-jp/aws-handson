# =============================================================================
# eventbridge.tf — cloudtrail-eventbridge-realtime
# CloudTrail の危険な API 操作を EventBridge ルールでリアルタイム検知し、SNS に転送する。
#
# 【イベントパイプラインの構成】
# CloudTrail（管理イベント）
#   → EventBridge デフォルトバス（CloudTrail 管理イベントは自動で配信される）
#     → EventBridge ルール（危険 API パターンでフィルタリング）
#       → SNS トピック
#         → メール通知
#
# 【CloudTrail と EventBridge の連携について】
# CloudTrail の管理イベント（コントロールプレーン操作）は、追加設定なしで
# EventBridge のデフォルトイベントバスに自動配信される。
# source = "aws.cloudtrail"、detail-type = "AWS API Call via CloudTrail" で
# イベントを受け取れる。
#
# 【検知対象の API 操作】
...
# 4. ルートユーザー操作
# ---------------------------------------------------------------------------
# ルートユーザーによるコンソールログインやAPI操作を検知する。
# ルートは MFA なしでも全権限を持つため、ルートの直接操作はそれ自体が異常シグナル。
# userIdentity.type = "Root" でフィルタリングする。
